  (* Мы хотели решить вашу задачу, но один участник команды заболел, а остальные два 
  отвелкались вузом и другой задачей по модел чекингу. К сожалению, успели прорешать частично только
  первые два задания. *)
...
Section AbsState.
  ...
  Definition vble  := @ble  V _.
  Definition vjoin := @join V _.

  Definition keys_list (m : StringMap.t V) : list string :=
  List.map (fun '(k, _) => k) (StringMap.elements m).

   Definition keys (m : StringMap.t V) : 𝒫 string :=
    fun x => In x (keys_list m).

  Definition keys_union' (m1 m2 : StringMap.t V) : 𝒫 string :=
    keys m1 ∪ keys m2.

  Definition keys_of (m : StringMap.t V) : list string :=  
    List.map (fun '(k, _) => k) (StringMap.elements m).  

  Definition ble_maps (m1 m2 : StringMap.t V) : bool :=
    let ks := List.app (keys_of m1) (keys_of m2) in  
    forallb (fun k => vble (getm k m1) (getm k m2)) ks.

  Definition join_maps (m1 m2 : StringMap.t V) : StringMap.t V :=
    StringMap.fold
      (fun k v acc =>
        let v' := vjoin v (getm k acc) in
  ...
  Instance astateLatticeOp : LatticeOp astate := {
    ...
    join x y :=
      ...
      | None   , a      => a      
      | a      , None   => a
      | Some m1, Some m2 => Some (join_maps m1 m2)
      end;
    ...
    ble x y :=
      ...
      | None   , _        => true          
      | Some _ , None     => false        
      | Some m1, Some m2  => ble_maps m1 m2
      end
  }.
  ...
  Let γV : V -> 𝒫 Z := @γ V lat_val Z γ_val.

  Definition γ_astate (a : astate) : 𝒫 state :=
    match a with
    | None     => fun _ => False
    | Some m   => fun st => forall x, st x ∈ γV (getm x m)
    end.
  ...
  Lemma ble_maps_spec' :
    forall m1 m2,
      ble_maps m1 m2 = true <->
      (forall x, x ∈ (keys m1 ∪ keys m2) ->
                vble (getm x m1) (getm x m2) = true).
  Proof.
    intros m1 m2. unfold ble_maps. unfold vble.                                                                                                                                                                                                                                                                                             

    (* apply forallb_forall.   *)
  Admitted.

  Lemma γ_astate_mon : 
    forall a b : astate, (a ≤? b) = true -> γ_astate a ⊆ γ_astate b.
  Proof.
    intros a b.
    unfold getm. unfold γ_astate. unfold γV. 
    destruct a.
    - destruct b. 
      + simpl. unfold ble_maps. intros F. intros a0.  
  Admitted.


  Lemma γ_astate_join :
    forall a b : astate, γ_astate a ∪ γ_astate b ⊆ γ_astate (a ∨ b).
  Proof.
  Admitted.

  Lemma γ_astate_bot :
    forall c : state, ~ γ_astate ⊥ c.
  Proof.
    auto.
  Qed.
  ...
  Lemma γ_astate_top :
    forall c : state, γ_astate ⊤ c.
  Proof.
    intros c. unfold γ_astate. simpl. 
    intros x. simpl. unfold getm. 
    simpl. unfold γV. apply γ_top.
  Qed.
...
Section fixpoint.
  ...
  Lemma iter_postfix_or_top :
    forall n a, iter n a = ⊤ \/ (f (iter n a)) ≤? (iter n a) = true.
  Proof.
    induction n as [|n IH]; intros a; simpl.
    - now left.
    - remember (f a) as a'.
      destruct (a' ≤? a) eqn:Ha.
      + right. simpl. subst a'. now rewrite Ha.
      + specialize (IH a'). destruct IH as [IHtop | IHpf].
        * left. simpl. exact IHtop.
        * right. simpl. exact IHpf.
  Qed.
  ...
  Proof.
    unfold postfixpoint.
    destruct (iter_postfix_or_top niter ⊥) as [Htop | Hle].
    - intros c _.
      rewrite Htop. apply γ_top.
    - intros c Hc.
      eapply γ_mon; [exact Hle | exact Hc].
  Qed.
...
Section Analysis.
  ...
  Proof.
    induction e.
    - intros F. simpl (aeval (Var x) a). simpl (eval (Var x) s). 
      unfold γ in F. simpl in F. unfold γ_astate in F. destruct a.
      + apply F.
      + contradiction.
    - intros F. cbn. apply γ_aconst.
    - intros F. cbn. apply γ_aunop. revert F. exact IHe. 
    - intros F. cbn. apply γ_abinop; revert F; assumption. 
  Qed.
  ...
  Fixpoint aceval (c : com) (a : astate V) : astate V :=
    ...
    | If e c1 c2 => join (aceval c1 a) (aceval c2 a)
    | While e c  => postfixpoint (fun X => join a (aceval c X))
    end.
  ...
  Proof.
    induction c; intros s s' a.
    - intros C. cbn.
      simpl in X. unfold γ_astate in X. destruct a.
      + admit.
      + contradiction.
    - cbn. 
  Admitted. 
